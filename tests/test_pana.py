...
def test_crypto_context():
    ...
    assert ctx.pana_pac_encr_key is not None
    assert ctx.pana_paa_encr_key is not None
...
def test_encryption():
    ...
    ctx.pana_pac_encr_key = b'0123456789abcdef'  # Also set the new key
...
def test_auth_verification_with_reserved():
    ...
    # RFC 5191 compliance: non-zero reserved field should be rejected
    parsed = PANAMessage()
    try:
        parsed.unpack(packed)
        # Should have failed due to non-zero reserved field
        assert False, "Should reject non-zero reserved field"
    except ValueError as e:
        assert "Reserved field not zero" in str(e)
        print("✓ RFC 5191 reserved field validation working")
        return  # Test successful - validation caught the error
    ...
    # This code is now unreachable due to RFC validation
