...
## 目次
1. [プロジェクト概要](#プロジェクト概要)
2. [アーキテクチャとリファクタリング](#アーキテクチャとリファクタリング)
3. [OpenPANA相互運用性](#openpana相互運用性)
4. [RFC 6786 AVP暗号化](#rfc-6786-avp暗号化)
5. [TLS鍵エクスポート](#tls鍵エクスポート)
6. [テストと検証](#テストと検証)
...
pyPANAは、RFC 5191で定義されているネットワークアクセス認証プロトコル（PANA - Protocol for carrying Authentication for Network Access）のPython実装です。本プロジェクトは、EAP-TLS認証をサポートするクライアント（PaC - PANA Client）とサーバー（PAA - PANA Authentication Agent）の両方の実装を提供します。

### 主要機能
- 正しい16バイトヘッダーフォーマットによるRFC 5191完全準拠
- RFC 6786準拠のAVP暗号化完全実装
- TLS鍵エクスポート付きEAP-TLS認証（RFC 5216）
- エンタープライズ認証のためのRADIUSバックエンド統合
- モジュール式で保守可能なアーキテクチャ
- RFC準拠テストを含む包括的なテストカバレッジ

### 現在の実装ステータス

#### ✅ 完全実装済み
- コアPANAプロトコル（RFC 5191） - 全メッセージタイプとステートマシン
  - 正しい16バイトヘッダー: Reserved(2) + Length(2) + Flags(2) + Type(2) + SessionID(4) + Sequence(4)
  - 正しいAVPフォーマット: Code(2) + Flags(2) + Length(2) + Reserved(2) + Value
- RFC 6786 AVP暗号化 - 完全実装
  - Encryption-Algorithm AVP（コード13）ネゴシエーション
  - Encryption-Encap AVP（コード12）による機密データ保護
  - RFC 6786準拠nonce形式のAES-128-CTR
  - 双方向暗号化（PANA_PAC_ENCR_KEYとPANA_PAA_ENCR_KEY）
- 暗号化アルゴリズム:
  - PRF_HMAC_SHA2_256による鍵導出
  - AUTH_HMAC_SHA2_256_128によるメッセージ認証（128ビット切り詰め）
  - AES128_CTRによるAVP暗号化
- 適切なMSK/EMSK導出を伴うEAP-TLS認証
- PyOpenSSL経由のTLS鍵エクスポート（RFC 5705）
- RADIUSバックエンド統合
- セッション管理と再送信
- メッセージ認証（AUTH AVP）
- リプレイ攻撃対策
...
#### ⚠️ 部分実装
- レート制限 - 実装済みだがデフォルトで無効
- OpenPANA相互運用性 - 基本的なメッセージ交換確認済み、セッション管理の違いが残存

#### ❌ 未実装
- 追加のEAPメソッド（TTLS、PEAP、MSCHAPv2）
- マルチキャスト経由のPAAディスカバリー
- IPモビリティサポート
- 64KB超のメッセージフラグメンテーション

---

## アーキテクチャとリファクタリング

コードベースは2000行以上の単一ファイルから、適切に構造化されたモジュール式パッケージにリファクタリングされました。

### モジュール構造

#### コアプロトコルモジュール
- **`pana_constants.py`** - RFC5191定数、フラグ、メッセージタイプ
- **`pana_messages.py`** - PANAメッセージとAVP構造
- **`pana_crypto.py`** - 暗号化操作と鍵導出
- **`eap_tls.py`** - RFC5216準拠のEAP-TLSハンドラー

#### インフラストラクチャモジュール
- **`pana_session.py`** - セッション管理とステートマシン
- **`pana_retransmission.py`** - Rフラグサポート付きメッセージ再送信
- **`pana_ratelimit.py`** - レート制限とDoS保護
- **`pana_monitor.py`** - セッション監視と統計

#### アプリケーションモジュール
- **`pana_client.py`** - PANAクライアント（PaC）実装
- **`pana_server.py`** - PANA認証エージェント（PAA）実装
- **`main.py`** - コマンドラインインターフェースエントリーポイント

#### 暗号化サポート（RFC 6786）
- **`pana_encryption_policy.py`** - 暗号化ポリシーとコンテキスト
- **`pana_client_encryption.py`** - クライアント側暗号化ヘルパー
- **`pana_server_encryption.py`** - サーバー側暗号化ヘルパー

### モジュール式アーキテクチャの利点
1. **保守性** - 明確なモジュール境界と単一責任
2. **テスト可能性** - 個々のモジュールを独立してテスト可能
3. **再利用性** - コンポーネントを独立してインポート・使用可能
4. **拡張性** - 既存コードに影響を与えずに新機能を追加可能
